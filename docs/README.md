...
Follow its Repository link, then load only the durable context directly required by that task. This
page is a router, not a mandatory reading list.
...
If a document has no Repository link from an admitted Notion Task, do not infer current work from it;
PRODUCT must select or create the Task in Notion.
