...
| Generated plan engine / refresh | Structured authoring, selected plans, active-plan refresh proposal/apply | rich workout fields, `plan_preferences`, blueprint/refresh review metadata, source status | Keep accepted plan-goal/block truth; debug/proof metadata can be pruned only after validators and QA no longer consume it. |
...
| `completeTextOnboarding` saved-mode text replacement seam | Keep until separate retirement decision | Non-default but current backend capability using canonical structured/rich workout normalization. |
...
| --- | --- | --- | --- |
| No-active-plan entry | Signed-in runners without an active plan see creation choices instead of silent preview assignment. | `route-data-actions`, `first-plan-actions`, `training-api` wrappers; `OnboardingGate` and onboarding components. | No silent plan creation; do not mark unresolved IA blockers as cleanup-safe. |
| Structured first plan / AI-authored blueprint draft | Runner enters profile/goal/training context, reviews backend-shaped draft, then confirms. | `first-plan-actions`, `structured-first-plan-onboarding`, `ai-first-plan-blueprint-*`, `active-plan-persistence`; validators under `scripts/validate-plan-authoring-doctrine.ts`. | No AI direct persistence; no fake pace/HR; no voice-to-plan unless rebuilt and QA-accepted. |
| Selected-distance plan creation | Runner creates accepted distance-goal plans after preview/review/create. | `running-plan-engine-actions`, `running-plan-engine-review`, `ai-generated-running-plan`, `active-plan-persistence`, `plan-creation-engine/*`; QA artifact `qa-artifacts/screenshots/2026-07-06/generated-plan-early-phase-dosing-qa/`. | Do not call AI at confirm; do not persist raw AI output; do not treat Marathon Base as current truth. |
...
| Persisted manual workout edit | Manual-only future-row editability is tracked as an accepted manual-authoring capability, but broader `Edit training` wording still needs current-doc reconciliation before cleanup agents use it as a deletion basis. | `edit-workout`, `edit-workout-review-token`, active-plan editability policy; workout detail edit UI. | No generated/selected/imported content editing and no today/past/logged/protected edits. |
...
| Hito inline editable text pattern | Shared inline editable/read-only primitive plus non-mutating local inspector task targeting. | `inline-editable-text`, devtool target utilities, manual constructor title, `/hitoDS/patterns`. | No generated workout editing or fake Admin Capture persistence. |
...
| Text replacement / refresh | Saved-mode text replacement and refresh remain backend review/apply seams. | `plan-replacement-actions`, `active-plan-refresh-actions`, `active-plan-refresh-draft`. | No AI direct mutation, silent refresh apply, or visible `Update plan` header action. |
...
- broad runner-facing `Edit training` beyond the accepted/manual-scoped future-row path
- generated/selected/imported persisted workout-content editing
