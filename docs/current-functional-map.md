...
- Manual constructor review/readback now uses backend-owned `constructorContract.timeline` for
  ...
  authored dated generated-plan path: 10K, 21.1K, 42.195K, and Custom 15K all preview/create saved
  plans with child-first repeats and W1-W4 conservative adaptation. Custom 15K persists as
  ...
  accepted through first generated non-rest workout logging and saved completed-state readback.
  Garmin/FIT provider upload and deterministic planned-vs-actual comparison remain a separate later
  QA gate.
...
| Hito inline editable text pattern | Internal/product UI can reuse one inline editable/read-only text pattern while local inspector task targeting creates non-mutating local prompt drafts. | Shipped DS primitives plus local-only devtool behavior; product usage limited to manual constructor title | No persistence owner for local inspector task targeting in v1; Admin Capture remains future owner only if Product/Backend later accept persisted change tasks | `src/components/ui/inline-editable-text.tsx`, `src/components/devtools/local-inline-change-target-utils.ts`, `src/components/manual-workout/ManualWorkoutConstructorEditor.tsx`, `/hitoDS/patterns#inline-editable-text` | No persisted inspector draft storage; browser localStorage stores only the local devtool enabled toggle; manual constructor edits stay draft-owned | Frontend/QA accepted inline pattern implementation | No generated workout content editing; no fake Admin Capture persistence; local inspector task targeting is non-mutating |
