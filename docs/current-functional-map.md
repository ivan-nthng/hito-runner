...
| Runner profile, settings, onboarding | Persisted baseline, accepted estimated/personal HR truth, training preferences, first-plan input | `src/components/OnboardingGate.tsx`, `src/components/onboarding/*`, `src/routes/settings.tsx`, `src/lib/user-settings-actions.ts`, `src/lib/heart-rate-zones.ts`, `src/lib/runner-training-preferences.ts` | Frontend collects backend-shaped input; backend owns validation, provenance, revision, persistence, and immutable plan-time snapshots. |
...
| Active-schedule date without a workout, including after the final scheduled workout | Render as `Rest` | A plan is a set of workouts rather than a runner-facing calendar boundary; do not add an after-plan/outside-plan state. |
...
| Manual template catalog | Runner saves, reuses, and deletes owned personal templates; built-ins remain metric-free global defaults with per-runner hide/restore state. | `template-catalog`, `saved-templates`, `saved-template-repository`, picker consumers, `runner_manual_workout_templates`, `runner_profiles.hidden_manual_workout_template_keys`. | No organization/coach templates or frontend-owned template rows. |
...
| Persisted workout content edit | Every confirmed non-rest workout on today or a future date enters one reviewed edit lifecycle regardless of source, logs, completion, or evidence; past rows remain non-editable. | `manual-workout-authoring/edit-workout`, edit review token, `active-plan-workout-editing/policy`, atomic persistence/history contract, source capabilities, workout detail UI. | No inline mutation or Rest editing. Review/confirm, auth, stale protection, provenance, and durable pre-edit history remain mandatory. |
