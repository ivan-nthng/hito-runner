...
- the visible onboarding UI now consumes the structured first-plan constructor contract:
  ...
  terrain focus can influence rolling or mountain/hill-oriented workouts without creating exact elevation targets or route matching, generated plans now carry exact session identity for tempo, interval, hill, cutback, and long-run steady-finish sessions inside the existing workout structure, recent 5K benchmark truth can produce broad runner-level pace targets on generated workout segments only when execution mode says the runner can follow watch/app pace guidance, and benchmark, terrain, target, execution mode, and optional comment context are treated only as bounded generation nuance rather than raw profile truth
  manual Quick setup now follows review-before-create: `generateStructuredFirstPlanDraft` returns a non-mutating runner-facing review, and `confirmStructuredFirstPlanDraft` is the explicit create seam that revalidates the reviewed draft, blocks existing active plans, and persists only after confirmation
...
- the internal structured authoring slice remains backend-only:
  it validates goal, schedule, runner basics, recent result context, available days, constraints, preferences, and execution mode, then generates one canonical `training-plan-v2` plan into the same persisted saved-mode seam without changing routes; generated workout details now avoid generic interval labels by carrying exact session identity, cutback-week simplification, and appropriate later long-run steady-finish structure while keeping compact calendar families stable
