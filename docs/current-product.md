...
- internal design-system reference `/hitoDS`
  uses a dedicated design-system sidebar and compact reference surface to document the simplified live Hito product language: semantic and primitive color tabs, canonical typography roles, the Hito icon registry, open route rhythm, divider-based grouping, restrained markers, quiet support copy, utility/disclosure patterns, bounded modal anatomy, controls, shell navigation, and documented visualization geometry exceptions; it is the internal reference baseline for future UI inspection and primitive extension, not a runner-facing capability
...
- the visible onboarding UI now consumes the structured first-plan constructor contract:
  ...
  normal longer easy, steady, cutback, taper, and long support runs are structured with clear opener/main/finish or equivalent purpose cues instead of defaulting to one anonymous main block; effort-only target-time plans therefore remain metric-safe without feeling like generic calendar filler, and low-support balanced marathon plans can vary safe recovery/cutback support identity without adding extra hard days
  ...
  manual Quick setup now follows review-before-create: `generateStructuredFirstPlanDraft` returns either inline corrections or a non-mutating runner-facing review modal, and `confirmStructuredFirstPlanDraft` is the explicit create seam that revalidates the reviewed draft, blocks existing active plans, and persists only after the modal primary confirmation
...
- `/admin/login` now provides a dedicated owner admin sign-in page:
  local/dev admin login can still use the protected local fixture, deployed/runtime admin login uses server-only password-hash configuration, valid tester/product credentials are refused with an admin-specific error instead of creating a normal product session, redirects stay limited to sanitized admin paths, and normal `/login`, Magic Link, and product local-login behavior remain unchanged; `/admin/analytics` admin-required states now point to this admin login path
