...
- Backend Slice 7 for admin Backlog work-item consolidation is implemented:
  `npm run import-admin-backlog-work-items` can dry-run or explicitly mirror approved Markdown work items into `admin_capture_items`. Canonical parsing now covers immutable `Work Item ID`, lifecycle, owner, scope, archive intent, optional batch, conditional frontend lane, task, stage, next role, and exact handoff. `Work Item ID` is the primary mirror identity; source type/path remains legacy fallback, duplicate IDs fail before writes, and one same-ID mirror survives a source move. Repo lifecycle maps without using `in_review`, legacy metadata debt stays explicit, and stale archival requires both absence of an approved source/same-ID replacement and the explicit `--archive-stale` action. Normal Admin mutations continue to reject repo-derived edits with `repo_derived_read_only`. Markdown normalization in the worktree is newer than the last accepted Admin refresh, so current Markdown remains authoritative; the generated non-mutating dashboard is only its active-plan projection until a separate live mirror gate is approved.
- Repository work-item lifecycle is now explicit: `docs/tasks/backlog/` alone owns operational
  `backlog`, `ready`, `in_progress`, `blocked`, and terminal truth. Product briefs, frontend specs,
  running-coach doctrine, and plans are linked supporting/history artifacts; their legacy status
  text cannot authorize dispatch. The Admin mirror preserves source groups and metadata debt, while
  the Markdown backlog remains authoritative until a separately approved live refresh.
