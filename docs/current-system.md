...
- `src/lib/structured-plan-authoring.ts`
  owns the first server-side structured authoring contract, its normalization rules, and the deterministic generator that emits canonical `training-plan-v2` plan truth before persistence; it now also accepts bounded `terrainFocus`, `ultra_marathon`, `mountain_running`, and execution-mode context, carries exact generated workout identity through `source_workout_type` plus titles, uses goal-family long-run floors/peaks/ceilings for half marathon, marathon, ultra, and mountain plans, applies runner-fit safety adjustments from age, benchmark presence, weekly frequency, and ambition/target pressure, selects different workout identities across `Base`, `Build`, `Specific`, and `Taper` phases, aligns long-run taper reductions with the same phase boundary so peak durability load happens before any `Taper` week, uses meaningful cutbacks, can split appropriate later long runs into easy-base plus steady-finish blocks, adds mountain/trail-specific identities for technical trail easy running, controlled downhill durability, hike-run endurance, climbing/rolling hill work, and mountain long-run time-on-feet without exact elevation prescriptions, adds goal-family quality identities for 5K short sharpening/strides, 10K rhythm intervals, half-marathon threshold durability, marathon steady specificity, and ultra time-on-feet durability, and uses one metric-mode resolver so generated `pace_min_per_km_range` targets appear only when the runner has watch/app access, asks for pace or mixed guidance, and has recent 5K benchmark truth
- `src/lib/openai-plan-authoring.ts`
  owns the first server-side OpenAI text-to-plan seam, prompts the model for bounded structured authoring input, validates that model output, and converts the validated authoring input into canonical `training-plan-v2` truth before persistence; the structured model contract now includes ultra/mountain goals, terrain focus, execution mode, and recent 5K pace truth so text authoring feeds the same deterministic generator contract as structured setup
