...
- hosted releases use `scripts/validate-supabase-deployment-parity.mjs` as the fail-closed schema boundary: the linked release check requires repository/hosted migration-history equality, while the Vercel prebuild reads the same canonical migration history through a service-role-only RPC and also requires the configured hosted Data API to expose the runner-baseline columns plus revision-aware plan persistence RPC before application build output can be accepted
...
- `src/routes/hitoDS.tsx`
  renders the production-shipped public Hito design-system reference and interactive sandbox with its own design-system navigation and no runner-facing shell chrome; it consumes canonical tokens, typography roles, shared UI primitives, and component contracts while keeping specimen interaction isolated from runner data, auth, providers, persistence, and Product business lifecycle
...
- `scripts/generate-hito-ds-manifest.mjs` remains the single code-owned generator for both generated manifest representations: `src/generated/hito-ds-manifest.ts` is the typed runtime import used by the reference and capture board, while `src/generated/hito-ds-manifest.json` is the machine-readable validator/downstream artifact. Both derive from the same foundation and typography owners, carry the same source digest, and stay separate because their current consumers are distinct rather than because they represent competing token truth.
