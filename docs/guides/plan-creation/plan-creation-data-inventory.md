...
1. User setup or Quick setup collects goal, schedule, profile, availability, constraints, and
   preferences.
2. `PlanGoalIntent` normalizes selected distance, target date, target finish time, and outcome
   identity without making coaching feasibility decisions.
3. Structured authoring input normalizes runner context and availability.
4. The AI provider or local fixture authors one self-describing `hito_ai_authored_full_plan_v1`
   response under the `ai_authored_plan_first_v1` source contract.
5. The backend `plan_first` service validates that response directly as the full draft, then compiles
   it through
...
| Target outcome pace                | Optional                          | `PlanGoalIntent.targetOutcomePace`                                                    | Optional                             | No                                                         | Backend may derive goal readback from finish time and distance | `plan-goal-intent.ts`                                                                                          | Goal identity only; AI authors every executable workout pace.                                                                                                   |
...
| Recent race / benchmark status     | Optional                          | benchmark setup                                                                       | Yes or absent                        | No                                                         | Absence remains absence                                     | `structured-plan-authoring-schema.ts`, provider context                                                         | AI may author estimated numeric pace without a benchmark; provenance remains `no_benchmark_ai_estimate`.                                                        |
...
| AI-authored phases/weeks/workouts  | Critical output                   | Provider/local fixture `hito_ai_authored_full_plan_v1`                                | No                                   | Yes                                                        | Backend validates/compiles                                  | `ai-authored-plan-first-provider-contract.ts`, `ai-first-plan-draft-service.ts`, `ai-authored-plan-first-compiler.ts` | Source kind is `ai_authored_plan_first_v1`; the full provider draft maps directly to compiler validation and remains non-persisted until review/confirm. |
| AI-authored steps/repeats          | Critical output                   | Direct provider `workouts[].sections[]` and `endpoint.sections[]`                     | No                                   | Yes                                                        | Backend compiles canonical workout/repeat structure         | `ai-authored-plan-first-compiler.ts`                                                                              | AI authors the workout structure; backend rejects structures it cannot safely display or persist.                                                               |
