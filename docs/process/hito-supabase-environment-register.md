...
| `hosted`    | Local production configuration resolves to hosted endpoint class and project ref `dltfjwexyctmihclcjqj`, matching the intended ref in [`scripts/validate-supabase-deployment-parity.mjs`](../../scripts/validate-supabase-deployment-parity.mjs); redacted endpoint fingerprint `sha256:cfbb3b6778a8f2ba`. The linked Vercel config names project `hito-runner`, with project fingerprint `sha256:bd7419e721db5595` and organization fingerprint `sha256:f3fd755c261edcb9`; target label is `production`. These are local configuration signals, not live-provider admission. | Current hosted application data and release target; personal/production data may exist                                                       | No read/write/reset/provider/deployment action from this register. A separately authorized hosted task must first complete admission.                                                                                                            | **Unresolved shared hosted boundary.** The accepted Git-backed application deployment `dpl_2u1gAPpRMVgwDg9dWcKnJWVtWjCd` corresponds to immutable application revision `fca161507e9dd344a141712d48d799b4204091d8`. HITO-302 revision `28cddb8f90b094c66eacae3d9a903a0889336b2b` adds only the undeployed `.prettierignore` cleanup and changes no migration or runtime source. The admitted HITO-303 recovery documentation/tooling release also changes no migration or runtime source and is not deployed. Live `main` is resolved from Git and the HITO-303 release receipt, never from this register. None of these revision facts proves the live CLI link, hosted application of all 59 repository migrations, generated-type parity, provider agreement, current writer, backup/restore or rollback readiness. No hosted Supabase parity is claimed. |
