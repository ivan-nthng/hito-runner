...
- Any bounded initial detail horizon, continuation rule, or goal-trajectory language is deferred
  until a separate Product decision after Runner Evidence and Progress acceptance.
...
6. **Adaptive Blueprint Planning decision** — after Evidence and Progress acceptance, decide whether
   to define a blueprint contract, detailed horizon, continuation preview, and next-block
...
- Adaptive planning remains intentionally undecided: initial horizon, continuation trigger,
  evidence threshold, and check-in policy are not implementation inputs yet.
