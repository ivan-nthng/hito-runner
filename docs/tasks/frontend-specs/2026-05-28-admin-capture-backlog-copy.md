...
Make `/admin/capture` read like a calm internal backlog, not a runner-facing feature, not a Jira clone, and not an automatic agent workflow.
...
| Page title | `Capture backlog` | Keep shorter than `Admin capture backlog`. |
| Page intro | `Review captured UI notes, triage items, and copy prompts for manual Codex handoff.` | States the three jobs of the page without implying auto-dispatch. |
| Secondary link to analytics | `Back to analytics` | Use only if the layout includes the optional return link. |
...
| Quick note trigger | `Quick note` | Matches design spec and distinguishes from UI capture. |
...
| Screenshot row marker | `Screenshot` | Future asset marker. |
| No screenshot marker | `No screenshot` | Use only when backend exposes asset absence explicitly. |
...
| Detail section title | `Captured context` | Matches design spec. |
| Detail section title | `Admin note` | |
| Detail section title | `Triage` | |
| Detail section title | `Assets` | |
| Detail section title | `Prompt handoff` | Keeps the action manual and explicit. |
| Detail section title | `Metadata` | |
...
| UI area | Recommended copy | Notes |
| --- | --- | --- |
| Dialog/disclosure title | `Quick note` | |
| Quick note intro | `Add a backlog item without selecting UI.` | Keeps it distinct from capture mode. |
...
| Failure state | `Copy failed` | Keep the label short. |
| Failure helper | `Could not copy. Try again.` | Avoid clipboard diagnostics in UI copy. |
...
| Admin unavailable | `Admin access required.` | `Sign in with an admin account to open the capture backlog.` | `Go to admin login` |
| Generic unavailable | `Capture backlog unavailable.` | `This admin tool is not available right now.` | `Back to analytics` |
...
| UI area | Recommended copy | Notes |
| --- | --- | --- |
| Assets section label | `Screenshots` | Use plural for a future multi-asset strip. |
| Single asset label | `Screenshot` | |
| Thumbnail action | `View screenshot` | Use only if preview/open behavior exists. |
| Asset-empty label | `No screenshot` | Quiet empty state. |
...
- Page intro: `Review captured UI notes, triage items, and copy prompts for manual Codex handoff.`
...
- Keep `Quick note` language separate from `Capture UI`. The quick note flow is not a capture flow.
...
- If screenshots are not part of the first shipped slice, should the UI hide `Assets` entirely when no asset support exists yet, or show the section with quiet `No screenshot` copy?
