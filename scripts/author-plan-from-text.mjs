...
const importedSeed = buildImportedPlanSeed(generatedPlan.canonicalPlan);
...
async function readAuthoringText(cliOptions) {
  if (cliOptions.prompt && String(cliOptions.prompt).trim()) {
    return String(cliOptions.prompt).trim();
  }

  if (cliOptions["prompt-file"] && String(cliOptions["prompt-file"]).trim()) {
    return (await readFile(String(cliOptions["prompt-file"]).trim(), "utf8")).trim();
  }
