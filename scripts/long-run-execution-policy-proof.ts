...
export function validateManualLongRunExecutionPolicyContract() {
  for (const minutes of [50, 60]) {
    assertManualReady(`manual continuous ${minutes}-minute long aerobic`, {
      templateKey: "long_aerobic_run",
      workoutDate: "2026-07-06",
      entries: [manualEntry("long_run_body_block", minutes)],
    });
  }
  assertManualRejected(
    "manual 95-minute runnable-support discriminator without Hydration",
    {
      templateKey: "long_aerobic_run",
      workoutDate: "2026-07-06",
      entries: [
        manualEntry("warmup_block", 5),
        manualEntry("long_run_body_block", 80),
        manualEntry("rest_walk_jog_recovery_block", 10),
      ],
    },
    /Long runs over 90 minutes/,
  );
  assertManualReady("manual 95-minute runnable-support discriminator with Hydration", {
    templateKey: "long_aerobic_run",
    workoutDate: "2026-07-06",
    entries: [
      manualEntry("warmup_block", 5),
  ...
  const manualHydrationReview = assertManualReady("manual four-hour checkpoint-separated anatomy", {
    templateKey: "long_aerobic_run",
    workoutDate: "2026-07-06",
    entries: [
      manualEntry("warmup_block", 5),
      manualEntry("long_run_body_block", 110, bpmManualTarget(z2Command)),
      manualHydration(),
      manualEntry("long_run_body_block", 120, bpmManualTarget(z2Command)),
      manualEntry("cooldown_block", 5),
    ],
  });
  const hydration = manualHydrationReview.draft.steps.find(
  ...
  assertManualRejected(
    "manual four-hour single-leaf anatomy",
    {
      templateKey: "long_aerobic_run",
      workoutDate: "2026-07-06",
      entries: [manualEntry("long_run_body_block", 240)],
    },
    /Long runs over 120 minutes/,
  );
  assertManualRejected(
    "persisted edit cannot bypass one-minute shell anatomy",
    {
      templateKey: "long_aerobic_run",
      workoutDate: "2026-07-06",
      entries: [manualEntry("warmup_block", 1), manualEntry("long_run_body_block", 61)],
    },
    /Long runs over 60 minutes/,
    { allowPersistedTemplateShape: true },
  );
  assertManualRejected(
    "manual one-minute shell anatomy",
    {
      templateKey: "long_aerobic_run",
      workoutDate: "2026-07-06",
      entries: [manualEntry("warmup_block", 1), manualEntry("long_run_body_block", 61)],
  ...
  assertManualRejected(
    "manual drills note is not a checkpoint",
    {
      templateKey: "long_aerobic_run",
      workoutDate: "2026-07-06",
      entries: [
        manualEntry("long_run_body_block", 70),
        {
          kind: "block",
          block: {
            blockKey: "drills_mobility_note_block",
            noteText: "Mobility after the run.",
          },
        },
        manualEntry("long_run_body_block", 30),
      ],
    },
    /Long runs over 90 minutes/,
  );
  assertManualRejected(
    "manual coach cue is not a checkpoint",
    {
      templateKey: "long_aerobic_run",
      workoutDate: "2026-07-06",
      entries: [
        manualEntry("long_run_body_block", 70),
        {
          kind: "block",
          block: {
            blockKey: "coach_cue_note_block",
            noteText: "Check posture.",
          },
        },
        manualEntry("long_run_body_block", 30),
      ],
  ...
  assertManualRejected(
    "manual Hydration cannot own duration",
    {
      templateKey: "long_aerobic_run",
      workoutDate: "2026-07-06",
      entries: [
    ...
    /fixed targetless non-runnable event/,
  );

  assertManualReady("manual controlled same-mode finish", {
    templateKey: "long_run_with_steady_finish",
    workoutDate: "2026-07-06",
    entries: [
      manualEntry("long_run_body_block", 60, bpmManualTarget(z2Command)),
      manualEntry("long_run_finish_block", 15, bpmManualTarget(z3Command)),
    ],
  });
  assertManualRejected(
    "manual mixed-mode finish",
    {
      templateKey: "long_run_with_steady_finish",
      workoutDate: "2026-07-06",
      entries: [
        manualEntry("long_run_body_block", 60, bpmManualTarget(z2Command)),
        manualEntry("long_run_finish_block", 15, paceManualTarget(paceTwo)),
      ],
    },
    /one substantive execution mode/,
  );
  assertManualReady("manual distance-based pace finish", {
    templateKey: "long_run_with_steady_finish",
    workoutDate: "2026-07-06",
    entries: [
      manualDistanceEntry("long_run_body_block", 12_000, paceManualTarget(paceOne)),
      manualDistanceEntry("long_run_finish_block", 3_000, paceManualTarget(paceTwo)),
    ],
  });
  assertManualRejected(
    "manual distance-based BPM finish",
    {
      templateKey: "long_run_with_steady_finish",
      workoutDate: "2026-07-06",
      entries: [
        manualDistanceEntry("long_run_body_block", 12_000, bpmManualTarget(z2Command)),
        manualDistanceEntry("long_run_finish_block", 3_000, bpmManualTarget(z3Command)),
      ],
    },
    /requires an authored pace command/,
  );

  assertManualReady("manual two-stage progression", {
    templateKey: "progression_run",
    workoutDate: "2026-07-06",
    entries: [
      manualEntry("warmup_block", 10),
      manualEntry("progression_block", 12, paceManualTarget(paceOne)),
      manualEntry("progression_block", 13, paceManualTarget(paceTwo)),
      manualEntry("cooldown_block", 5),
    ],
  });
  assertManualRejected(
    "manual four-stage progression",
    {
      templateKey: "progression_run",
      workoutDate: "2026-07-06",
      entries: [
        manualEntry("warmup_block", 10),
        manualEntry("progression_block", 6, paceManualTarget(paceOne)),
        manualEntry("progression_block", 6, paceManualTarget(paceTwo)),
        manualEntry("progression_block", 6, paceManualTarget(paceThree)),
        manualEntry("progression_block", 7, paceManualTarget("5:10-5:30/km")),
        manualEntry("cooldown_block", 5),
      ],
    },
  ...
  for (const templateKey of [
    ...
    const reviewed = assertManualReady(`default ${templateKey}`, {
      templateKey,
      workoutDate: "2026-07-06",
    });
    assert.ok(reviewed.reviewToken);
    assert.ok(reviewed.reviewChecksum);
    assert.equal(reviewed.draft.persisted, false);
  }

  const progression = getManualWorkoutTemplate("progression_run");
  assert.equal(
    progression.defaultEntries.reduce(
      (total, entryValue) =>
        total + (entryValue.kind === "block" ? (entryValue.block.durationSeconds ?? 0) : 0),
      0,
    ),
    40 * 60,
    "The three-stage progression default must preserve the prior 40-minute total.",
  );
}
...
function assertManualReady(label: string, input: ManualWorkoutDraftInput) {
  const reviewed = reviewManualWorkoutDraft(input);
  assert.equal(reviewed.ok, true, reviewed.ok ? "" : `${label}: ${JSON.stringify(reviewed)}`);
  if (!reviewed.ok) throw new Error(`${label} unexpectedly failed.`);
  return reviewed;
}

function assertManualRejected(
  label: string,
  input: ManualWorkoutDraftInput,
  expectedMessage: RegExp,
  options: Parameters<typeof reviewManualWorkoutDraft>[1] = {},
) {
  const reviewed = reviewManualWorkoutDraft(input, options);
  assert.equal(reviewed.ok, false, `${label} must fail before review or persistence.`);
  if (reviewed.ok) return;
  assert.match(JSON.stringify(reviewed.issues), expectedMessage);
  assert.equal("reviewToken" in reviewed, false);
}

function manualEntry(
  blockKey: ManualWorkoutBlockInput["blockKey"],
  durationMinutes: number,
  target?: ManualWorkoutBlockInput["target"],
...
function manualHydration(): ManualWorkoutConstructorEntryInput {
  return {
    kind: "block",
    block: {
      blockKey: "hydration_block",
...
function paceManualTarget(command: string): NonNullable<ManualWorkoutBlockInput["target"]> {
  return { paceMinPerKmRange: command };
}

function bpmManualTarget(command: string): NonNullable<ManualWorkoutBlockInput["target"]> {
  return { hrBpmRange: command };
}
