...
const RUNTIME_URL = readOption("--runtime-url") ?? "http://127.0.0.1:3000";
const ROLE = requireQaPoolRole(readOption("--role") ?? "adaptive-training-quality");
...
try {
  ...
  const account = accounts.find(
    (candidate) => candidate.username === QA_TESTER_POOL[ROLE].username,
  );
  assert.ok(account, `The canonical ${ROLE} account must exist.`);
  ...
  if (READ_ONLY) {
    assert.equal(restored.review.workoutDocuments.length, 28);
    const supabase = createAdminSupabaseClient();
    const [calendar, confirmations, usage] = await Promise.all([
      supabase
    ...
    let foreignPoolLoginRejected = false;
    if (ROLE === "camelot") {
      ...
      foreignPoolLoginRejected = true;
    }

    console.log(
      JSON.stringify({
        ok: true,
        boundary: "public_saved_plan_list_restore_readback",
        role: ROLE,
        listedReviewCount: listed.records.length,
        restoreStatus: restored.status,
        canonicalWorkoutDocumentCount: restored.review.workoutDocuments.length,
        confirmationCount: confirmations.count,
        calendarRowCount: calendar.count,
        externalProviderDispatchCount: usage.count,
        foreignPoolLoginRejected,
      }),
    );
  } else {
