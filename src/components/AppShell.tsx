...
import { HitoUiLocaleProvider } from "@/components/ui/hito-ui-locale-provider";
...
import { WEEK_STATUS_META, getShellSnapshot, type TrainingSnapshot } from "@/lib/training";
...
const NAV: { to: string; label: HitoProductMessageKey; icon: HitoIconName }[] = [
  { to: "/", label: "Calendar", icon: "calendar" },
  { to: "/progress", label: "Progress", icon: "progress" },
];

const WEEK_STATUS_MESSAGE_KEYS = {
  on_track: "On track",
  partially_off_track: "Partially off track",
  needs_reset: "Needs reset",
} as const satisfies Record<keyof typeof WEEK_STATUS_META, HitoProductMessageKey>;
...
export function AppShell({
  ...
  children: ReactNode | ((locale: ResolvedUiLocale) => ReactNode);
  ...
  const message = (key: HitoProductMessageKey) => getHitoProductMessage(resolvedLocale, key);
  const modeLabel = message(modeLabelKey);
  const profileName = viewer?.name ?? message(fallbackProfileNameKey);
  const profileDetail = message(getProfileDetailKey(snapshot, shellSnapshot.mode));
  ...
  const saveLanguagePreference = async (preference: UiLocalePreference) => {
    ...
    try {
      ...
      hitoToast.error({
        title: message("Language preference not saved"),
        description:
          error instanceof Error ? error.message : message("Try choosing the language again."),
      });
    } finally {
  ...
  return (
    <HitoUiLocaleProvider locale={resolvedLocale}>
      <div className="min-h-screen flex bg-background text-foreground">
        ...
        <aside className="hito-shell-sidebar-width hidden h-screen min-h-0 shrink-0 self-start box-border flex-col overflow-hidden border-r border-hairline bg-sidebar supports-[height:100dvh]:h-dvh [@media(min-width:48rem)_and_(min-height:32rem)]:sticky [@media(min-width:48rem)_and_(min-height:32rem)]:top-0 [@media(min-width:48rem)_and_(min-height:32rem)]:flex">
          <div className="shrink-0 pb-10 pr-6 pl-[max(var(--space-6),env(safe-area-inset-left))] pt-[max(1.75rem,env(safe-area-inset-top))]">
            <Link to="/" reloadDocument={useFreshHomeRequest} aria-label={message("Hito home")}>
            ...
            {NAV.map((navItem) => {
              ...
              return (
                ...
                >
                  ...
                  <span>{message(navItem.label)}</span>
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <HitoButton
                        ...
                        aria-label={
                          shellSnapshot.source === "persisted"
                            ? message("Dismiss Beta User note")
                            : message("Dismiss preview note")
                        }
                      >
                    ...
                    <p className="hito-body-sm mt-1 text-secondary">
                      {shellSnapshot.source === "persisted"
                        ? message("All features are free for you.")
                        : message(
        ...
        <main className="flex-1 min-w-0 flex flex-col bg-background dark:bg-surface">
            ...
            <div className="flex h-14 items-center gap-hito-4 px-hito-4 sm:gap-6 sm:px-6 lg:px-10">
              ...
              <div className="hidden items-baseline gap-3 [@media(min-width:48rem)_and_(min-height:32rem)]:flex">
                ...
                <span className="hito-technical-sm text-secondary">
                  {formatUiDate(shellSnapshot.currentDate, resolvedLocale, {
                    month: "long",
                    day: "numeric",
                    weekday: "long",
                  })}
              ...
              <div className="ml-auto flex items-center gap-3">
                {shellSnapshot.mode === "authenticated" ? (
                  <StatusPill
                    label={message("Week")}
                    tone={weekStatusTone(shellSnapshot.weekStatus)}
                    value={message(WEEK_STATUS_MESSAGE_KEYS[shellSnapshot.weekStatus])}
                  />
                ) : null}
                ...
                <HitoButton
                  asChild
                  aria-label={message("Open Connections")}
                  className="[@media(min-width:48rem)_and_(min-height:32rem)]:hidden"
                  iconOnly
                  size="sm"
                  variant="ghost"
                ...
                {showSettingsAction && (
                  <HitoButton
                    asChild
                    aria-label={message("Open profile and heart-rate settings")}
                    className="[@media(min-width:48rem)_and_(min-height:32rem)]:hidden"
                    iconOnly
                    size="sm"
                    variant="ghost"
          ...
          <div className="flex-1">
            {typeof children === "function" ? children(resolvedLocale) : children}
          </div>
...
function weekStatusTone(status: keyof typeof WEEK_STATUS_META): "success" | "warning" {
  return status === "on_track" ? "success" : "warning";
}
