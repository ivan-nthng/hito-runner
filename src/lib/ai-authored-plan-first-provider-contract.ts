...
export const AI_AUTHORED_PLAN_FIRST_PROVIDER_CONTRACT_VERSION =
  "adaptive-blueprint-four-week-direct-v21" as const;
...
export function buildAiAuthoredPlanFirstPrompt({
  ...
  const systemPrompt = [
    ...
    "Before returning, audit cadence mechanically. For each Blueprint phase, count every non-rest detailed workout in each complete calendar week inside detailed_block and count every projection in each future phase-week slice. Every count must equal that phase.expected_weekly_cadence except a partial boundary slice with fewer available dates. If a count is smaller, add the missing Easy or Recovery workout/projection slot or lower the phase cadence to the truthful exact count; never return a mismatched cadence.",
