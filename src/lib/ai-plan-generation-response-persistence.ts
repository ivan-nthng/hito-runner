...
export async function recordAiPlanGenerationTechnicalRecompileForUser(input: {
  userId: string;
  responseRecordId: string;
  sourceCompilerVersion: `adaptive_continuation_compiler_v${number}`;
  sourceDiagnosticCode: string;
  recompileCompilerVersion: `adaptive_continuation_compiler_v${number}`;
}): Promise<AiPlanGenerationResponseRow> {
  const response = await getAiPlanGenerationResponseForUser(input.userId, input.responseRecordId);
  const providerAttempt = jsonObject(response?.provider_attempt ?? null);
  const attemptResult = jsonObject(response?.attempt_result ?? null);
  const versionContext = jsonObject(response?.version_context ?? null);
  const compilerRecompiles = Array.isArray(providerAttempt?.compilerRecompiles)
    ? providerAttempt.compilerRecompiles.filter(
        (entry): entry is Json => Boolean(entry) && typeof entry === "object",
      )
    : [];
  const exactExisting = compilerRecompiles.some((entry) => {
    const record = jsonObject(entry);
    return (
      record?.sourceCompilerVersion === input.sourceCompilerVersion &&
      record?.sourceDiagnosticCode === input.sourceDiagnosticCode &&
      record?.recompileCompilerVersion === input.recompileCompilerVersion &&
  ...
  if (
    ...
    versionContext?.compilerVersion !== input.sourceCompilerVersion ||
    (response.compiler_outcome === "accepted" && !exactExisting) ||
    (response.compiler_outcome === "rejected" &&
      response.diagnostic_code !== input.sourceDiagnosticCode)
  ) {
    throw new Error("The retained technical response cannot record this compiler recompile.");
  }
  if (response.compiler_outcome === "accepted" && exactExisting) return response;

  const recompileEntry = toJson({
    sourceCompilerVersion: input.sourceCompilerVersion,
    sourceOutcome: "rejected",
    sourceDiagnosticCode: input.sourceDiagnosticCode,
    recompileCompilerVersion: input.recompileCompilerVersion,
    outcome: "accepted",
  ...
  const updated = await createAdminSupabaseClient()
    .from("ai_plan_generation_responses")
    .update({
      compiler_outcome: "accepted",
      diagnostic_code: null,
      diagnostic_path: null,
      provider_attempt: toJson({
        ...(providerAttempt ?? {}),
        compilerRecompiles: [...compilerRecompiles, recompileEntry],
      }),
    })
    .eq("id", input.responseRecordId)
    .eq("user_id", input.userId)
  ...
  if (!updated.data) {
    throw new Error("The retained technical response changed before its compiler recompile.");
  }
  return updated.data;
}
