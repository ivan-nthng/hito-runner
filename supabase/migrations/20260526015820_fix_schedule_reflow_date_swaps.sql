...
begin
	...
	loop
		...
		if exists (
			select 1
			from public.planned_workouts target_workout
			where target_workout.plan_cycle_id = p_plan_id
				and target_workout.workout_date = v_to_date
				and target_workout.id <> v_workout_id
				and not exists (
					select 1
					from jsonb_array_elements(p_updates) as moved(update_payload)
					where (moved.update_payload->>'workoutId')::uuid = target_workout.id
				)
		) then
			return jsonb_build_object(
				'ok', false,
	...
	loop
		v_workout_id := (v_update->>'workoutId')::uuid;
		v_parking_index := v_parking_index + 1;

		update public.planned_workouts
		set workout_date = date '9999-01-01' - v_parking_index,
			display_order = v_parking_index
		where id = v_workout_id
			and user_id = p_user_id
			and plan_cycle_id = p_plan_id;
		...
		if v_row_count <> 1 then
			raise exception 'schedule reflow parking failed for workout %', v_workout_id;
		end if;
	end loop;
