...
alter table public.adaptive_training_detailed_candidates
	add constraint adaptive_training_detailed_candidates_owner_response_fkey
		foreign key (user_id, source_response_id)
		references public.ai_plan_generation_responses (user_id, id)
		on delete cascade;

drop function public.retain_adaptive_training_continuation_candidate(
	uuid,
	uuid,
	integer,
	text,
...
begin
	...
	if not exists (
		...
		raise exception 'The accepted retained continuation response was not found for its owner.';
	end if;
...
comment on column public.adaptive_training_detailed_candidates.source_response_id is
	'Private immutable lineage to the accepted owner response that authored this detailed candidate. Historical pre-authoring candidates remain null and are never reviewable.';
...
comment on function public.retain_adaptive_training_continuation_candidate(
	...
	'Atomically retains one immutable owner-bound continuation candidate linked to its accepted private response. It never writes Runner Calendar rows.';
