				...
				or (
					...
					and (
						response.request_context
							#- '{initialPlanProfile,asOf}'
							#- '{initialPlanProfile,snapshotId}'
					) = (
						p_input_snapshot
							#- '{initialPlanProfile,asOf}'
							#- '{initialPlanProfile,snapshotId}'
...
comment on function public.retain_adaptive_training_source_candidate(
	...
	'Retains one immutable owner-bound initial candidate from either its originally accepted response or the narrowly proven immutable alias-only first-plan recompile; creates no Calendar or confirmation rows.';
